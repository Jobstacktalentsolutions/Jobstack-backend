...
} from '@app/common/shared/interfaces/auth-result.interface';
import {
...
export class RecruiterAuthService {
  ...
  constructor(
    ...
    private recruiterAuthRepository: Repository<RecruiterAuth>,
    @InjectRepository(RecruiterProfile)
    private recruiterProfileRepository: Repository<RecruiterProfile>,
    @InjectRepository(RecruiterSession)
    private recruiterSessionRepository: Repository<RecruiterSession>,
  ...
  async register(
    ...
    try {
      ...
      this.logger.log(`Recruiter registered: ${auth.id} (${email})`);
  ...
  async login(loginData: LoginDto, deviceInfo?: any): Promise<AuthResult> {
    ...
    this.logger.log(`Recruiter logged in: ${auth.id} (${email})`);
  ...
  async refreshToken(
    ...
    try {
      ...
      // Validate session
      const session = await this.recruiterSessionRepository.findOne({
        where: {
      ...
      await this.recruiterSessionRepository.save(session);
      ...
      this.logger.log(`Token refreshed for recruiter: ${auth.id}`);
  ...
  async logout(
    ...
    try {
      ...
      await this.recruiterSessionRepository.update(sessionId, {
        isActive: false,
        lastActivityAt: new Date(),
      });

      this.logger.log(`Recruiter logged out: session ${sessionId}`);
    } catch (error) {
  ...
  async resetPassword(
    ...
    try {
      ...
      await this.recruiterSessionRepository.update(
        { recruiter: { id: auth.id } },
        { isActive: false },
      );
  ...
  async validateSession(
    ...
    try {
      ...
      const dbSession = await this.recruiterSessionRepository.findOne({
  ...
  private async generateTokens(
    ...
    // Create session
    const session = this.recruiterSessionRepository.create({
      recruiter: auth,
